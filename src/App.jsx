...
function App() {
  ...
  return(
    ...
    <p className='text-2xl font-bold'>base64 encoder/decoder</p>
      ...
      <textarea id='input' className='h-48 w-full border p-2 border-gray-200' placeholder='input' />
      </div>
        <div className='w-full lg:w-1/2 px-2 pt-0 sm:pt-2 pb-2'>
        <textarea id='output' className='h-48 w-full border p-0 border-gray-200' placeholder='output' />
        </div>

      </div>
      <div className='sm:full'>
      <button className='my-2 mx-2 bg-gray-200 px-3 py-2' onClick={() => Encode()}>Encode</button>
        <button className='my-2 bg-gray-200 px-3 py-2' onClick={() => Decode()}>Decode</button>
      </div>
    </div>
    </div>
  );
